...
type ContractStructure struct{
	...
	Contract_Money int `json:"Contract_Money"`
	...
	Importer_Check bool `json:"Importer_Check"`
	Exporter_Check bool `json:"Exporter_Check"`
	Custom_Check bool `json:"Custom_Check"`
	Importer_Bank_Check bool `json:"Importer_Bank_Check"`
	Insurance_Check bool `json:"Insurance_Check"`
}
...
func (s *SimpleChaincode) create_Contract(stub shim.ChaincodeStubInterface, args []string) pb.Response{
	...
	contractMoney,_:= strconv.Atoi(args[2])
	contract := ContractStructure{Contract_Id:args[0], Content_Description:args[1], Contract_Money:contractMoney,  Exporter_Id:args[3], Importer_Id:args[4], Importer_Bank_Id:args[5],Insurance_Id:args[6],Custom_Id: args[7], Port_Of_Loading:args[8], Port_Of_Entry:args[9], Importer_Check: false, Exporter_Check: true, Custom_Check: false, Importer_Bank_Check: false, Insurance_Check: false}
...
// 	contract_id:=args[0]
// 	exporter_Account:=args[1]
// 	contractByte, _ := stub.GetState(contract_id)
// 	if contractByte == nil {
// 		return shim.Error("Account number Invalid ")
// 	}
// 	contract_Struct:=ContractStructure{}
// 	errorAccount := json.Unmarshal(contractByte, &contract_Struct)
// 	if errorAccount != nil {
// 		return	shim.Error("something wrong")
// 	}
// 	exporterByte, _ := stub.GetState(exporter_Account)
// 	if exporterByte == nil {
// 		return shim.Error("Account number Invalid ")
// 	}
// 	accountStruct:=AccountStructure{}

// 	error := json.Unmarshal(exporterByte, &accountStruct)
// 	if error != nil {
// 		return shim.Error("something wrong")
// 	}
// 	if accountStruct.Account_Number!=contract_Struct.Exporter_Id{
// 		return shim.Error("invalid exporter")
// 	}
// 	contract_Struct.Exporter_Check=true
// 	fmt.Println(contract_Struct)
// 	contract,_ := json.Marshal(contract_Struct)
// 	fmt.Println(contract)
// 	erroraccount := stub.PutState(contract_id,contract)
// 	fmt.Println(contract_Struct.Contract_Id+"contract")
// 	if erroraccount != nil {
// 		return shim.Error("something wrong")
// 	}
// 	//response:="success"
...
func (s *SimpleChaincode) accept_By_Importer(stub shim.ChaincodeStubInterface, args []string) pb.Response{
	...
	contractAsByte, _ := stub.GetState(contract_id)
	if contractAsByte == nil {
		return shim.Error("contract number Invalid ")
	}
	contract_Struct:=ContractStructure{}
	errorContract := json.Unmarshal(contractAsByte, &contract_Struct)
	if errorContract != nil {
		return	shim.Error("something wrong")
	}
	accountAsByte, _ := stub.GetState(account_Number)
	...
	accountSructure := AccountStructure{}
	errorAccount := json.Unmarshal(accountAsByte, &accountSructure)
	if accountSructure.Account_Balance<contract_Struct.Contract_Money{
	...
	contract_Struct.Importer_Check=true
	contract,_ := json.Marshal(contract_Struct)
	error := stub.PutState(contract_Struct.Contract_Id,contract)
	if error != nil {
		return shim.Error("something wrong")
	}
	//response:="success"
...
func (s *SimpleChaincode) accept_By_Custom(stub shim.ChaincodeStubInterface, args []string) pb.Response{
	...
	account_Number:=args[1]
	 ...
	 errorContract_Custom := json.Unmarshal(contractAsByte, &contractSruct)
	 if errorContract_Custom != nil {
		return shim.Error("Something wrong")
	}
	 customAccountAsByte, _ := stub.GetState(account_Number)
	 if customAccountAsByte == nil {
		return shim.Error("exporter_Id is Invalid ")
	}
	 accountStruct:=AccountStructure{}
	 errorAccount_Custom := json.Unmarshal(customAccountAsByte, &accountStruct)
	 if errorAccount_Custom != nil {
		return shim.Error("Something wrong")
	}
     if accountStruct.Account_Balance < contractSruct.Contract_Money{
	    return shim.Error("invalid amount")
     }
	 contractSruct.Custom_Check=true
	 contract,_ := json.Marshal(contractSruct)
	 error := stub.PutState(contractSruct.Contract_Id,contract)
	if error != nil {
		return shim.Error("something wrong")
	}
	 return shim.Success(contract)
...
func (s *SimpleChaincode) accept_By_ImporterBank(stub shim.ChaincodeStubInterface, args []string) pb.Response{
	contract_id :=args[0]
	imprterbankaccount_Number := args[1]
	contractAsByte, _ := stub.GetState(contract_id)
	if contractAsByte == nil {
		return shim.Error("contract number is Invalid")
	}
	contract_Struct:=ContractStructure{}
	errorContract := json.Unmarshal(contractAsByte, &contract_Struct)
	if errorContract != nil {
	...
	accountAsByte, _ := stub.GetState(imprterbankaccount_Number)
	if accountAsByte == nil {
	...
	if accountSruct.Account_Balance<contract_Struct.Contract_Money{
		return shim.Error("account balance is less than 10000")
	}
	//errorAccount := json.Unmarshal(accountAsByte, &accountSruct)
	
	contract_Struct.Importer_Bank_Check=true
	contract,_ := json.Marshal(contract_Struct)
	error := stub.PutState(contract_Struct.Contract_Id,contract)
   if error != nil {
	   return shim.Error("something wrong")
   }
	s.moneyTransfer_By_Exporter(stub, contract_id )
//	response:="success"
	return shim.Success(contract)
}
...
func (s *SimpleChaincode) accept_By_Insurance(stub shim.ChaincodeStubInterface, args []string) pb.Response{
	 ...
	 contractSruct := ContractStructure{}
	 errorContract_Insurance := json.Unmarshal(customAsByte, &contractSruct)
	 accountAsByte, _ := stub.GetState(account_Number)
	 if errorContract_Insurance != nil {
		return shim.Error("something wrong")
	}
     accountstruct:=AccountStructure{}
     errorAccount := json.Unmarshal(accountAsByte, &accountstruct)
     if errorAccount != nil {
	   return	shim.Error("something wrong")
	  }
	 if accountstruct.Account_Balance<contractSruct.Contract_Money{
	 ...
	 contractSruct.Insurance_Check=true
	 contract,_ := json.Marshal(contractSruct)
	    error := stub.PutState(contractSruct.Contract_Id,contract)
        if error != nil {
	   return shim.Error("something wrong")
...
func (s *SimpleChaincode) moneyTransfer_By_Exporter(stub shim.ChaincodeStubInterface,args string) pb.Response{
	...
	exporter_Bal=exporter_Bal+contractStruct.Contract_Money
	...
	importer_Bal=importer_Bal-contractStruct.Contract_Money
	importer_Bal=importer_Bal-2000
